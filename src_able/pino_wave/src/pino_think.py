	...
	while not rospy.is_shutdown():
		if listen_sentence != listen_init:
			if listen_sentence == listen_reset: #listen nothing
				answer_message_for_able.data = answer_sound_reset
				...
				if answer != -1: #matching
					...
					answer_message_for_able.data = sound
				
				else: #no matching
					rand = random.randrange(1,101)
					print("what are you saying?")
					if rand <= 30:
						answer_message_for_able.data = 48
					else:
						answer_message_for_able.data = answer_sound_reset
			...
			rospy.loginfo(answer_message_for_able)
			able_node_status = True
			pub_able_script.publish(answer_message_for_able)
			pub_able_status.publish(able_node_status)
			reset()
		rospy.sleep(0.1)
	print "pino_think_node is dead"
	able_node_status = False
	pub_able_status.publish(able_node_status)